...
$referencia_pago1 = cleanStringConverterCase($_POST['ben_nm']);
$referencia_pago2 = "";
$referencia_pago3 = "";
$activo           = 1;

// CONSULTAR TIPO FACTURA
$q = "SELECT tipo_factura FROM facturas WHERE facturas_id = '$facturas_id'";
...
// SECUENCIA (si tuviera)
$qs = "SELECT secuencia_facturacion_id FROM facturas WHERE facturas_id  = '$facturas_id'";
$rs = $mysqli->query($qs) or die($mysqli->error);
if($rs->num_rows>0){ $secuencia_facturacion_id = $rs->fetch_assoc()['secuencia_facturacion_id']; }
...
if($tipo_factura === "1"){ // CONTADO
    ...
    if($rc->num_rows>0){
        ...
        $pagos_id = correlativo('pagos_id','pagos');
        $mysqli->query("INSERT INTO pagos VALUES ('$pagos_id','$facturas_id','2','$fecha','$efectivo_bill','0','$cambio','$efectivo_bill','$usuario','$estado_pago','$empresa_id','$fecha_registro')");
