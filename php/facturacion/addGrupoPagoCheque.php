...
$query_factura = "SELECT servicio_id, colaborador_id, fecha, pacientes_id, tipo_factura FROM facturas_grupal WHERE facturas_grupal_id = ?";
...
if($tipo_factura === "1"){
    ...
    if($result_saldo_cxc->num_rows>0){
        ...
        $query_pagos = "SELECT CAST(COALESCE(SUM(importe), 0) AS 'importe' FROM pagos_grupal WHERE facturas_grupal_id = ?";
