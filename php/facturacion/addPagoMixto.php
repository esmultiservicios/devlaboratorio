...
$referencia_pago1 = cleanStringConverterCase($_POST['cr_bill']);
$referencia_pago2 = cleanStringConverterCase($_POST['exp']);
$referencia_pago3 = cleanStringConverterCase($_POST['cvcpwd']);
...
if($tipo_factura === "1"){
    ...
    if($rp->num_rows==0){
        ...
        $mysqli->query("INSERT INTO pagos_detalles VALUES ('$pagos_detalles_id','$pagos_id','$tipo_pago_id','$banco_id','$importe','$referencia_pago1','$referencia_pago2','$referencia_pago3')");
        // factura pagada
        ...
        if($rc->num_rows>0){
            ...
            $nuevo_saldo = $saldo_cxc - (float)$importe;
...
}else{ // CREDITO (abono mixto, pero se registra como un importe total)
    $rc = $mysqli->query("SELECT saldo FROM cobrar_clientes WHERE facturas_id='$facturas_id' AND estado=1") or die($mysqli->error);
    if($rc->num_rows>0){
        $saldo_cxc = (float)$rc->fetch_assoc()['saldo'];
        $nuevo_saldo = $saldo_cxc - (float)$efectivo_bill;

        $pagos_id = correlativo('pagos_id','pagos');
        // Guardamos el abono total en 'importe' y 'efectivo' (según tu diseño de pagos), y en tarjeta el mismo total si quieres conservar "no-efectivo"
        $mysqli->query("INSERT INTO pagos VALUES ('$pagos_id','$facturas_id','2','$fecha','$efectivo_bill','$efectivo_bill','0','$tarjeta','$usuario','$estado_pago','$empresa_id','$fecha_registro')");
        // detalle
        $pagos_detalles_id = correlativo('pagos_detalles_id','pagos_detalles');
        $mysqli->query("INSERT INTO pagos_detalles VALUES ('$pagos_detalles_id','$pagos_id','$tipo_pago_id','$banco_id','$efectivo_bill','$referencia_pago1','$referencia_pago2','$referencia_pago3')");
